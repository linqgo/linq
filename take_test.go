...
func TestTake(t *testing.T) {
	...
	assertHave(t, 5, data.Take(999).FastCount)
	assertLack(t, oneshot().Take(999).FastCount)
}
...
func TestTakeLast(t *testing.T) {
	...
	assertHave(t, 5, data.TakeLast(999).FastCount)
	assertLack(t, oneshot().TakeLast(999).FastCount)
}
...
func TestTakeWhile(t *testing.T) {
	...
	assertHave(t, 0, linq.None[int]().TakeWhile(linq.True[int]).FastCount)
	assertLack(t, data.TakeWhile(linq.True[int]).FastCount)
	assertLack(t, oneshot().TakeLast(999).FastCount)
}
...
func TestTakeElementAt(t *testing.T) {
	...
	assertHave(t, 2, maybe(linq.Take(linq.From(1, 2, 3, 4, 5), 3).FastElementAt(1)))
	assertLack(t, maybe(linq.Take(linq.From(1, 2, 3, 4, 5), 3).FastElementAt(3)))
}
