...
func TestSkip(t *testing.T) {
	...
	assertHave(t, 5, data.Skip(0).FastCount)
	assertHave(t, 3, data.Skip(2).FastCount)
	assertLack(t, oneshot().Skip(0).FastCount)
}
...
func TestSkipLast(t *testing.T) {
	...
	assertHave(t, 5, data.SkipLast(0).FastCount)
	assertHave(t, 2, data.SkipLast(3).FastCount)
	assertLack(t, oneshot().SkipLast(0).FastCount)
}
...
func TestSkipWhile(t *testing.T) {
	...
	assertLack(t, data.SkipWhile(linq.False[int]).FastCount)
	assertLack(t, oneshot().SkipWhile(linq.False[int]).FastCount)
}

func TestSkipElementAt(t *testing.T) {
	t.Parallel()

	assertHave(t, 5, maybe(linq.Skip(linq.From(1, 2, 3, 4, 5), 3).FastElementAt(1)))
	assertLack(t, maybe(linq.Skip(linq.From(1, 2, 3, 4, 5), 3).FastElementAt(3)))
}
